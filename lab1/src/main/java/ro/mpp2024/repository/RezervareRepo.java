...
public class RezervareRepo implements IRepository<Integer, Rezervare> {
    ...
    public void save(Rezervare entity) {
        ...
        Connection con=dbUtils.getConnection();
        try(PreparedStatement preStmt=con.prepareStatement("insert into Rezervare values (?,?,?,?,?)")){
            preStmt.setInt(1,   entity.getId().intValue());
            preStmt.setInt(2,entity.getExcursie().getId().intValue());
            preStmt.setString(3,entity.getNumeClient());
            preStmt.setString(4,entity.getNrTelefon());
    ...
    public void update(Integer integer, Rezervare entity) {
        ...
        try(PreparedStatement preparedStatement=con.prepareStatement("update Rezervare set  excursie = ?, " +
            ...
            preparedStatement.setInt(1, entity.getExcursie().getId().intValue());
        ...
        try(PreparedStatement preStmt=con.prepareStatement("select * from Rezervare where id=?")){
            ...
            try(ResultSet result=preStmt.executeQuery()) {
                if (result.next()) {
                    ...
                    Rezervare rez = new Rezervare(exRepo.findOne(id_excursie), nume_client, numar_telefon, numar_locuri);
                    logger.traceExit();
                    return rez;
                }
            }
